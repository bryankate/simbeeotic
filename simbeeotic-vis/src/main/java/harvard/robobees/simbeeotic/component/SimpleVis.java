...
public class SimpleVis extends Frame implements VariationComponent, MotionListener {
	...
	private static Point3d userPosn = new Point3d(20,20,12);
	...
	private void orbitControls(Canvas3D c){
	  ...
	  // Set the user's initial viewpoint using lookAt()
	  {
	    ...
	    t3d.lookAt(userPosn, new Point3d(0,0,3), new Vector3d(0,0,1));
	    t3d.invert();
	...
	public void initializeObject(int objectId, CollisionShape shape) {
		...
		// cylinder shape
			...
			drawCylinder(objectId, pink, r, Math.abs(2*halfExtents.z));
		...
		// box shape
			...
			drawBox(objectId, gray, Math.abs(2*halfExtents.x), Math.abs(2*halfExtents.y), Math.abs(2*halfExtents.z));
			u.addBranchGraph(objRoot);
		}	
		// cone shape
		...
		// plane shape
			...
			drawBox(objectId,green,6000,0,6000);
...
//		Vector3d pos = new Vector3d(position.x+.1,position.y+.1,position.z+.1);
//		objRoot.addChild(makeText(pos,"" + objectId));
//		u.addBranchGraph(objRoot);
	...
	private void drawSphere(int objectId, Color3f color, float radius) {
		...
		//appearance 
		Material yellowMat = new Material(color,black,color,specular,25.0f);
...
//	    objRoot.addChild(Tg);
	...
	private void drawCone(int objectId, Color3f color, float r, float h) {
		Material brownMat = new Material(color,black,color,specular,25.0f);
		brownMat.setLightingEnable(true);
		Appearance brownApp = new Appearance();
		brownApp.setMaterial(brownMat);
	...
	private void setUserPosn(Point3d userPosn) {
		userPosn = userPosn;
	}
