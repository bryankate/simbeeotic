...
import harvard.robobees.simbeeotic.comms.MessageReceivedEvent;
...
public abstract class SimpleBee extends AbstractPhysicalModel {
    ...
    // parameters
    private double updateRate = 0.1;             // seconds
     ...
     *
     * @param currTime The time at which the logic is applied.
     */
    protected abstract void applyLogic(SimTime currTime);


    /**
     * Called when a message is received by this model's radio.
     *
     * @param currTime The time at which the message is received.
     * @param message The message payload.
     */
    protected abstract void handleMessage(SimTime currTime, byte[] message);
    

    /**
     * {@inheritDoc}
     *
     * This implementation schedules the first periodic update event.
     */
    @Override
    public void initialize() {
        ...
        // kick everything off
        scheduleEvent(new SimTime(0), new UpdateEvent());
    }
    ...
    protected RigidBody initializeBody(DiscreteDynamicsWorld world) {
        ...
        // setup comms
        radio = new DefaultRadio(getCommModel(), getSimEngine(), this, radioPowerMax);
        getCommModel().addRadio(radio);
    ...
    /**
     * An event handler that invokes the custom logic, updates the bee's motion,
     * and schedules the next periodic update.
     *
     * @param time The time of the update.
     * @param event The update event (not used).
     */
    @EventHandler
    public final void update(final SimTime time, final UpdateEvent event) {
        ...
        // recycle the old event instance since it is not really sued anyway
        scheduleEvent(new SimTime(time.getTime() + updateRate), event);
    }


    /**
     * An event handler for incoming messages.
     *
     * @param time The time of the message.
     * @param event The message received on the radio.
     */
    @EventHandler
    public final void messageReceived(final SimTime time, final MessageReceivedEvent event) {
        handleMessage(time, event.getData());
    }
     ...
     *
     * @param vel The desired linear velocity of the bee (m/s, in the body frame).
     */
    ...
    public final void setUpdateRate(@Named(value = "update-rate") final double rate) {
